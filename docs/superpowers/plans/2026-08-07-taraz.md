...
**Architecture:** One Next.js (App Router, TypeScript) application. Three isolated layers: `lib/sms/` holds pure parsing functions with no I/O, `lib/db/` owns the schema and every query, and `app/` holds thin route handlers and React UI. Deployed to Vercel with Turso (libSQL) as the database.

**Tech Stack:** Next.js 15 (App Router), TypeScript, Tailwind CSS v4, Drizzle ORM + `@libsql/client`, vaul (bottom sheet), jalaali-js, bcryptjs, Vitest, Vercel.
...
- Node 20+.
- **Vercel's filesystem is ephemeral.** Nothing may be written to disk at runtime. Every read and write goes through the libSQL client.
- **Every `lib/db/queries.ts` function is `async` and must be awaited.** libSQL is network-backed even when pointed at a local file. A forgotten `await` yields a `Promise` where a row is expected, and TypeScript will catch it only if the call site is typed — so run `npx tsc --noEmit` after every task that touches the database.
- Local development and tests use `TURSO_DATABASE_URL=file:./data/taraz.db`, so the suite never needs the network or a Turso account.
...
| `lib/db/client.ts` | libSQL client and Drizzle instance |
...
| `app/(app)/add/page.tsx` | Manual entry and Paste SMS recovery |
...
| `scripts/backup.sh`, `README.md` | Turso backups and setup documentation |
...
# Local development and tests: a plain file, no account needed.
TURSO_DATABASE_URL=file:./data/taraz.db
# Production only. Leave empty for file: URLs.
TURSO_AUTH_TOKEN=
```
...
- Produces:
  - `isDuplicate(bodyHash: string, now: number): Promise<boolean>`
  - `logSms(input: { rawText: string; bodyHash: string; sender: string | null; receivedAt: number; parseOk: boolean; error?: string | null }): Promise<number>`
  - `insertParsed(parsed: ParsedSms, smsLogId: number, receivedAt: number): Promise<number>`
  - `insertNeedsReview(smsLogId: number, receivedAt: number): Promise<number>`
  - `listInbox(): Promise<(Transaction & { categoryName: string | null })[]>`
  - `listCategories(): Promise<Category[]>`
  - `setCategory(txId: number, categoryId: number): Promise<void>`
  - `clearCategory(txId: number): Promise<void>`
  - `insertManual(input: { amount: number; direction: 'debit' | 'credit'; categoryId: number | null; note: string | null; occurredAt: number }): Promise<number>`
  - `updateTransaction(txId: number, patch: { amount?: number; direction?: 'debit' | 'credit'; categoryId?: number | null; note?: string | null; occurredAt?: number }): Promise<void>`
  - `deleteTransaction(txId: number): Promise<void>`
  - `monthSummary(startMs: number, endMs: number): Promise<{ total: number; byCategory: { categoryId: number | null; name: string; icon: string; total: number }[]; daily: { day: number; total: number }[] }>`
  - `listByCategory(startMs: number, endMs: number, categoryId: number): Promise<Transaction[]>`
  - `hashBody(raw: string): string`
  - `formatToman(rial: number): string`

Every function that touches the database is `async`. `hashBody` and
`formatToman` stay synchronous.
...
### Task 13: Manual entry and Paste SMS

**Files:**
- Create: `lib/ingest.ts`, `app/(app)/add/page.tsx`, `app/(app)/add/AddForm.tsx`, `app/(app)/add/PasteSms.tsx`
- Modify: `app/api/sms/route.ts` (delegate to `ingestSms`), `app/(app)/actions.ts` (add `pasteSmsAction`)
- Test: `lib/ingest.test.ts`

**Interfaces:**
- Consumes: `listCategories`, `addManualAction`, `parse`, `hashBody`, `isDuplicate`, `logSms`, `insertParsed`, `insertNeedsReview`
- Produces:
  - `ingestSms(text: string, sender: string | null, now: number): Promise<'parsed' | 'needs_review' | 'duplicate'>`
  - `pasteSmsAction(text: string): Promise<'parsed' | 'needs_review' | 'duplicate'>`

The Paste SMS box is the recovery path for anything the webhook missed while
Vercel or Turso was unreachable. The original message is still in the Messages
app, so pasting it produces exactly the transaction the webhook would have.
Both paths must behave identically, so the ingest logic moves out of the route
handler into `lib/ingest.ts` and the route becomes a thin wrapper.

The manual form takes toman, since that is what the user reads off a receipt,
and converts to rial before storing.

- [ ] **Step 1: Write the failing test for the shared ingest function**
...
Expected: FAIL — `@/lib/ingest` not found.

- [ ] **Step 3: Write the shared ingest function**
...
export async function ingestSms(
  ...
  const bodyHash = hashBody(text)
  if (await isDuplicate(bodyHash, now)) return 'duplicate'
...
- [ ] **Step 5: Rewrite the route handler to delegate**

In `app/api/sms/route.ts`, replace the imports of the query functions and the
whole block from `const bodyHash = hashBody(text)` to the end of `POST` with:
...
Run: `npx vitest run app/api/sms/route.test.ts`
Expected: PASS, 7 tests — unchanged behavior, since the route's contract did not move.

- [ ] **Step 6: Add the paste server action**
...
- [ ] **Step 8: Write the manual entry form**
...
- [ ] **Step 9: Write the Paste SMS box**

```tsx
// app/(app)/add/PasteSms.tsx
'use client'
...
Then paste a Saman SMS into the Paste SMS box and tap Import.
Expected: "Saved. Check the Inbox to categorize it." and the transaction is in the Inbox. Pasting the same text again immediately reports "Already recorded."
...
```bash
git add "app/(app)/add" "app/(app)/actions.ts" app/api/sms/route.ts lib/ingest.ts lib/ingest.test.ts
git commit -m "feat: add manual entry and Paste SMS recovery"
```
...
### Task 15: Vercel deployment, Turso, and backups
...
- [ ] **Step 2: Apply migrations to Turso from your machine**
...
Expected: `migrations applied`. Migrations are never run from a Vercel function — concurrent short-lived instances would race.
...
Then set the environment variables for Production, Preview, and Development:
...
Generate the secrets first: `openssl rand -hex 32` for `SMS_API_KEY` and `COOKIE_SECRET`, and `npx tsx scripts/hash-password.ts '<your password>'` for `APP_PASSWORD_HASH`.

Expected: a live `https://<project>.vercel.app`. Visiting it redirects to `/login`.
...
Expected: a dated `.sql` file containing your schema and rows.

This matters more than usual here: if the Turso or Vercel account is ever restricted, these dumps are the only copy of your data that you control.
...
npm run db:migrate                                  # against the local file: URL
npm run dev
```

Local development uses `TURSO_DATABASE_URL=file:./data/taraz.db`. No Turso
account is needed to run the app or the tests.
...
# Migrate from your machine — never from a Vercel function.
TURSO_DATABASE_URL='libsql://...' TURSO_AUTH_TOKEN='...' npm run db:migrate

npx vercel --prod
```

Environment variables in the Vercel project: `SMS_API_KEY`,
`APP_PASSWORD_HASH`, `COOKIE_SECRET`, `TURSO_DATABASE_URL`,
`TURSO_AUTH_TOKEN`.

After any schema change: `npx drizzle-kit generate`, commit, then run
`npm run db:migrate` against Turso **before** deploying the code that needs it.

Schedule backups on a machine you control:
`0 3 * * * cd ~/taraz && ./scripts/backup.sh`

### Known risk

Vercel and Turso may restrict Iranian traffic or accounts. If either becomes
unreachable, SMS arriving during the outage is lost at capture — the iOS
Shortcut fires once and does not retry. Recover with the **Paste SMS** box on
the Add screen; the original messages are still in the Messages app. Keep the
`scripts/backup.sh` dumps current so an account closure is survivable.
...
Open **Add → Paste SMS**, paste the bank message from Messages, tap Import. It
runs through the same parser as the webhook and produces the same transaction.
...
Run the `curl` from the README against the production URL.
Expected: `{"ok":true,"status":"parsed"}`, and the transaction appears in the Inbox after signing in.

- [ ] **Step 7: Run the full suite and commit**

Run: `npm test && npx tsc --noEmit && npm run build`
Expected: every test passes, no type errors, build succeeds.

```bash
git add scripts/backup.sh README.md
git commit -m "feat: add Vercel deployment, Turso backups, and setup docs"
```
...
- [ ] The production site loads on the iPhone over mobile data with the VPN **off** — this is the assumption the whole deployment rests on, so test it before relying on it
